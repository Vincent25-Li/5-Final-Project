...
import math
import csv
import sys #for 'sys.stdout.flush()' => 逐步列印
import os #for file path 

###django###
from django.conf import settings ###for file upload
from django.shortcuts import render, redirect
from django.http import HttpResponse #from class
from django.template import loader # from class #test

# sys.path.append('../')
# from studentsapp import load_data #read load_data.py
...
#         # style_path = os.path.join(settings.MEDIA_ROOT, 'jessie','blogs', 'original') #img path 
#         # GAN = CycleGAN(image_folder=style_path) #物件實體化,指定img_forder的argument
#         # GAN.load_model_and_weights(GAN.G_B2A)
#         # GAN.load_model_and_generate_synthetic_images()
        
#         # #after img processed, return back to user interface
#         # img_filepath = os.path.join(settings.MEDIA_ROOT, 'jessie','blogs', 'transfer')
...
class CycleGAN():
    def __init__(self, lr_D=2e-4, lr_G=2e-4, image_shape=(256*1, 256*1, 3),
        ...
        self.discriminator_iterations = 1  # Number of discriminator training iterations in each training loop
        self.beta_1 = 0.5 #for adam
        self.beta_2 = 0.999 #for adam
        self.batch_size = 1 #每次都要update loss fxn! => 為了更新Gen. & Dis. =>also because of Instance Norm.
        ...
        if self.use_multiscale_discriminator: 
            ...
            D_A = self.modelDiscriminator() #define in below
            D_B = self.modelDiscriminator() #define is below
            loss_weights_D = [0.5]  # 0.5 since we train on real and synthetic images
        # D_A.summary()

        # Discriminator builds
        image_A = Input(shape=self.img_shape)   #Input E Keras fxn
        image_B = Input(shape=self.img_shape)   #Input E Keras fxn
        guess_A = D_A(image_A)
        guess_B = D_B(image_B)
        ...
        # Generators
        self.G_A2B = self.modelGenerator(name='G_A2B_model') #define in below
        self.G_B2A = self.modelGenerator(name='G_B2A_model') #define in below
        # self.G_A2B.summary()
        ...
        synthetic_B = self.G_A2B(real_A)    #假Ｂ
        synthetic_A = self.G_B2A(real_B)    #假Ａ
        dA_guess_synthetic = self.D_A_static(synthetic_A)
        dB_guess_synthetic = self.D_B_static(synthetic_B)
        reconstructed_A = self.G_B2A(synthetic_B)   #重構Ａ
        reconstructed_B = self.G_A2B(synthetic_A)   #重構Ｂ
        ...
        self.writeMetaDataToJSON()  #define in "Save"
...
# Models
    ...
    def modelDiscriminator(self, name=None):
        ...
        ### Layer 5
        x = self.ck(x, 1024, True)
        ### Layer 6
        x = self.ck(x, 2048, True)
        # Output layer
    ...
    def train(self, epochs, batch_size=1, save_interval=1):
        def run_training_iteration(loop_index, epoch_iterations):
            ...
            if self.use_linear_decay and epoch > self.decay_epoch:
                self.update_lr(self.D_A, decay_D)       #update_lr define in "Help fxns"
            ...
            if loop_index % 20 == 0:
                ...
                self.print_ETA(start_time, epoch, epoch_iterations, loop_index) #define in "Help fxns"
        ...
        synthetic_pool_A = ImagePool(self.synthetic_pool_size) #實體化  #self.synthetic_pool_size => init para.
        synthetic_pool_B = ImagePool(self.synthetic_pool_size) #實體化  #self.synthetic_pool_size => init para.   
        ...
        if self.use_linear_decay:
            decay_D, decay_G = self.get_lr_linear_decay_rate()  #define in "Help fxns"
        ...
        for epoch in range(1, epochs + 1):
            ...
            #save images every epoch (svae_interval = 1)
            if epoch % save_interval == 0:
                print('\n', '\n', '-------------------------Saving images for epoch', epoch, '-------------------------', '\n', '\n')
                self.saveImages(epoch, real_images_A, real_images_B)    #define in 'Help fxns'
    ...
    def load_model_and_generate_synthetic_images(self):
        #response = input('Are you sure you want to generate synthetic images instead of training? (y/n): ')[0].lower()
            ...
            def save_image(image, name, domain):
                print(f'domain : {domain}') #domain : A
...
class ImagePool():  #used to update the discriminators
